<template>
  ...
  <!-- 回复列表 -->
  <!-- van-action-sheet 上拉菜单组件 -->
  <!-- @closed="reply.commentId=null 当退出之后讲评论id重置为null -->
...
export default {
  data () {
    return {
      ...
      // 控制提交中状态数据
      submiting: false,
      comments: [], // 用来寻访评论列表的数据
      offset: null, // 表示分页一句 如果为空，表示从第一页开始
      showReply: false, // 控制回复列表组件的显示和隐藏
      reply: {
        // 专门用reply这个对象存放回复次昂管的数据
  ...
  methods: {
    // 提交评论的方法
    async submit () {
      if (!this.value) return false // 表示如果当前评论内容为空就立即返回
      ...
      // 对评论进行评论
        ...
        const data = await commentOrReply({
          ...
          art_id: this.reply.commentId ? this.$route.query.articleId : null // 如果此时对文章进行评论 art_id不能传 如果是对 评论进行评论 文章ID必须传
        })
    ...
    async getReply () {
      ...
      this.reply.list.push(...data.results) // 讲评论添加到数据尾部
      // 拿到的data只是请求的第一页的数据
